...
import { FileText, ArrowLeftRight, Loader2, AlertCircle } from 'lucide-react';
...
export function DocumentCompare() {
  ...
  const handleCompare = async () => {
    ...
    try {
      const result = await compareAPI.compareDocuments(doc1Id, doc2Id, {
        comparisonType: 'full',
      ...
      <CardContent className="space-y-6">
          ...
          <div className="space-y-4 pt-4 border-t">
            ...
            {comparison.summary && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="p-4 bg-green-50 rounded-lg">
                  <div className="text-2xl text-green-700">{comparison.summary.additions}</div>
                  <div className="text-sm text-green-600">Additions</div>
                </div>
                <div className="p-4 bg-red-50 rounded-lg">
                  <div className="text-2xl text-red-700">{comparison.summary.deletions}</div>
                  <div className="text-sm text-red-600">Deletions</div>
                </div>
                <div className="p-4 bg-yellow-50 rounded-lg">
                  <div className="text-2xl text-yellow-700">{comparison.summary.modifications}</div>
                  <div className="text-sm text-yellow-600">Modifications</div>
                </div>
                <div className="p-4 bg-blue-50 rounded-lg">
                  <div className="text-2xl text-blue-700">{comparison.summary.totalChanges}</div>
            ...
            {comparison.summary && (
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span>{(comparison.summary.similarityScore * 100).toFixed(1)}%</span>
                </div>
                <Progress value={comparison.summary.similarityScore * 100} />
              </div>
            )}
            ...
            {comparison.changes && comparison.changes.length > 0 && (
              <div className="space-y-3">
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {comparison.changes.slice(0, 10).map((change: any) => (
                        ...
                        <div className="flex-1">
                          ...
                          {change.location && (
                            <p className="text-xs text-gray-500 mt-1">
                              Page {change.location.page} - {change.location.section}
                            </p>
                          )}
                        </div>
                      </div>
            ...
            {comparison.diffUrl && (
              <div className="flex gap-2">
                <Button variant="outline" onClick={() => window.open(comparison.diffUrl, '_blank')}>
                  View Diff Document
                </Button>
                {comparison.sideBySideUrl && (
                  <Button
                    variant="outline"
                    onClick={() => window.open(comparison.sideBySideUrl, '_blank')}
                  >
                    View Side-by-Side
                  </Button>
                )}
        ...
        {!comparison && !comparing && doc1Id && doc2Id && (
          <div className="text-center py-8 text-gray-500">
            ...
            <p>Click "Compare Documents" to analyze differences</p>
          </div>
        )}
      </CardContent>
    </Card>
